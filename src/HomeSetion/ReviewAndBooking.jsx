...
const ReviewAndBooking = () => {
  ...
  // Animation hooks
  const [ref1, inView1] = useInView({ triggerOnce: true, threshold: 0.2 }); // Heading text
  const [ref2, inView2] = useInView({ triggerOnce: true, threshold: 0.2 }); // Testimonial Card & Booking
  ...
  return (
    <section className="relative bg-[#052429] lg:p-30 !md:pt-25 bg-no-repeat object-cover overflow-clip bg-left  py-16">
      ...
      <div className='absolute hidden lg:block top-0 object-cover w-[32%] h-[100%] -left-32'><img src={officeCleaningImg} alt="" className='object-cover h-[100%]' /></div>
      ...
      <div className="container mx-auto px-4 md:px-12">
        <div className="flex flex-col md:flex-row gap-10 md:gap-0 items-start">
          ...
          <div
            ref={ref2}
            className={`w-full lg:w-4/11 bg-green-600 bg-opacity-90 gap-5 z-10 py-20 md:px-20 px-5 transition-all duration-700 ease-in-out ${inView2 ? "slide-up-fade" : "opacity-0 translate-y-8"}`}
            style={{
          ...
          >
            <div className='flex flex-row gap-2 items-center'>
              ...
              <span className="text-lg text-white font-bold uppercase">Cleaning Service</span>
            </div>
          ...
          <div className="w-full lg:w-6/11 flex flex-col overflow-hidden z-10 justify-center items-center gap-5 text-white">
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className='border-2 border-dashed rounded-full border-green-600'>
                  <img src={testimonial.image} alt="Profile" className="w-22 h-22 object-cover rounded-full" />
                </div>
                <div>
                  <div className="flex items-center text-green-700">
                    {[...Array(5)].map((_, i) => (
                      <i key={i} className={`text-xl mr-1 ${i < testimonial.rating ? 'fas fa-star' : 'far fa-star'}`}></i>
                    ))}
                  </div>
            ...
            >
              <div className='ml-20 flex gap-5 p-5 bg-[#052429] flex-row'>
                ...
                <div className='border-2 border-dashed border-[#146673] rounded-full'>
                  <button onClick={handleNext} className="bg-[#0b3c43] hover:bg-green-700 text-white text-3xl flex justify-center items-center h-15 w-15 rounded-full">
                    <GoArrowRight />
                  </button>
                </div>
              </div>
